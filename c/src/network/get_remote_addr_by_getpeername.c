...
int main(int argc, char *argv[])
{
    ...
    ret = getpeername(connfd, (struct sockaddr*)&remote_addr, &remote_addr_len);
    assert(ret == 0);
    ...
    ret = getsockname(connfd, (struct sockaddr*)&server_addr, &server_addr_len);
    assert(ret == 0);
    char server_ip[INET_ADDRSTRLEN];
    inet_ntop(AF_INET, (void *)&server_addr.sin_addr, server_ip, INET_ADDRSTRLEN);
    printf("server ip: %s, sever_port:%d\n", server_ip, ntohs(server_addr.sin_port)); /* server ip: 127.0.0.1, sever_port:9999 */
